...
1. **보행 정책으로 파지 시 로봇 전복 문제**
   - **문제 상황**: Spot 로봇이 소화기 등을 파지하기 위해 팔을 뻗을 때, 무게 중심이 전방으로 쏠리면서 기존 보행 정책이 무너지고 로봇이 전복되는 현상이 잦았습니다.
   - **해결책**: 이 문제는 로봇의 다리와 팔 관절 액추에이터의 강성(Stiffness) 및 댐핑(Damping) 파라미터를 파지 모션 진입 시점과 무인 운송 상태에 맞춰 다리는 2000, 팔과 그리퍼는 5000 수준으로 개별 정밀 튜닝하여 전방 지지력을 물리적으로 대폭 확보함으로써 전복 없이 균형을 완벽히 잡도록 해결했습니다.

2. **Nav2 코스트맵 튜닝 - 목표 근접 시 과보정 및 제자리 폭주**
   - **문제 상황**: 로봇이 목표 지점 근처에 도달했을 때, 미세한 위치 오차를 계속 보정하려다 보니 제자리에서 빙빙 돌며 폭주하는 문제가 발생했습니다.
   - **해결책**: 이 문제는 `applications/spot_policy.py` (또는 `spot_agent.py` 연동부) 스크립트에 `idle_brake` 로직을 추가하여 해결했습니다. 로봇이 목표의 일정 오차 범위 내에 진입하면, 강제로 목표에 최종 도달한 것으로 판정하고 구동 제어 신호를 차단(Brake)하여 제어를 깔끔하게 종료시키도록 수정했습니다.

3. **Nav2 이동 시 과도한 회전 및 관절 움직임**
   - **문제 상황**: Nav2 경로를 따라 이동할 때 로봇이 목표를 향해 너무 급격하게 회전하거나 관절이 튀는 등 모션이 불안정했습니다.
   - **해결책**: 이를 부드럽게 만들기 위해 `applications/spot_agent.py` 내의 `_stable_drive_command` 연산 파트에서 과거의 제어 명령과 현재 명령을 적절히 섞어주는 저역통과필터(Low-pass filter)를 설계했습니다. `self._smoothed_cmd`(또는 `self._safe_drive_command`) 변수를 통해 급격한 명령 속도 변화(Slew rate)를 상쇄시켜 관절 동작을 부드럽게 연쇄 보간하였습니다.
