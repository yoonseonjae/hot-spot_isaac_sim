...
class SpotArmFlatTerrainPolicy(PolicyController):
    ...
    def load_hybrid_policies(self, walking_policy_path: str, balance_policy_path: str, arm_balance_policy_path: str = None, policy_params_path: str = None) -> None:
        ...
        class MLPModel(nn.Module):
            def __init__(self, obs, obs_groups, obs_set, output_dim, hidden_dims, activation="elu", obs_normalization=False):
                super().__init__()
                ...
                for hidden_dim in hidden_dims:
                    ...
                    curr_in = hidden_dim
                layers.append(nn.Linear(curr_in, output_dim))
                self.actor_mlp = nn.Sequential(*layers)
                
            def forward(self, obs_dict):
                return self.actor_mlp(obs_dict["policy"])
            ...
            def load_state_dict(self, state_dict, strict=False):
                ...
                import torch
                sd_tensors = [v for k, v in state_dict.items() if isinstance(v, torch.Tensor) and len(v.shape) > 0 and 'std' not in k]
                
                if len(sd_tensors) >= len(own_keys):
                    for i, key in enumerate(own_keys):
                        own_state[key].copy_(sd_tensors[i])
                else:
        ...
        import os
        if balance_policy_path and os.path.exists(balance_policy_path):
            ...
            print(f"[Warning] Balance policy not found at {balance_policy_path}. Falling back to walking policy for balance mode.")
            self.balance_policy = self.walking_policy
        
        # 3. Load Arm Balance Policy (76-dim)
        if arm_balance_policy_path and os.path.exists(arm_balance_policy_path):
